...
def main():
    ...
    create_summaries(cross_entropy, "loss")
    #create_summaries(ta, "ta")
    ...
    test_writer = tf.summary.FileWriter(result_dir + "/valid/", sess.graph)
    ...
    for i in range(max_step):
        ...
        if i % 1100 == 0:		
             # print test error
             test_summary_str, valid_acc= sess.run([summary_op, accuracy], feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
             #ta = accuracy.eval(feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
             #test_summary_str, valid_acc= sess.run([summary_op, accuracy], feed_dict={x: valid_batch[0], y_: valid_batch[1], keep_prob: 1.0})
             print("validation accuracy: %.4f" % valid_acc)
             
             #print("test accuracy %g"%test_summary_str)
